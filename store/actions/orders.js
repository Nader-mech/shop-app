...
export const fetchOrders = () => {
    return async (dispatch,getState) =>{
        ...
        try{
            const response = await fetch(`https://rn-complete-guide-6c263.firebaseio.com/orders/${userId}.json`)
            if(!response.ok){
...
export const  addOrder = (cartItems , totalAmount) => {
    ...
    return async  (dispatch, getState) =>{
         ...
         const userId = getState().auth.userId
        const response = await fetch(`https://rn-complete-guide-6c263.firebaseio.com/orders/${userId}.json?auth=${token}`, {
            method: 'POST',
            headers: {
