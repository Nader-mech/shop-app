...
import OrderItem from '../../compenents/shop/OrderItem'
...
if(isLoading){
    return<View style={StyleSheet.centered}><ActivityIndicator size='large' color={Colors.primaryColor} /></View>
}
if (orders.length === 0) {
    return (
    ...
    return (
        <FlatList data={orders} keyExtractor={item => item.id} renderItem={itemData =>
             <OrderItem items={itemData.item.items} amount={itemData.item.totalAmount} 
